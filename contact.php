...
$password = strip_tags(trim($_POST['password']));
    ...
    $sql = $db->prepare('INSERT INTO accesstable(user, password) VALUES(:user, SHA(:password = "$password"))'); 
...
$password = strip_tags(trim($_POST['password']));
    ...
    $sql = $db->prepare('DELETE AccessTable(user, password) VALUES(:user, SHA(:password = "$password"))'); 
    $sql->bindValue(':user', $user, PDO::PARAM_STR);
    $sql->bindValue(':password', $password, PDO::PARAM_STR); 
    $sql->execute();
